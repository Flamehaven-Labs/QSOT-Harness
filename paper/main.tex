...
\begin{abstract}
We present \texttt{QSOT-Harness} (QSOT: \emph{Quantum State Over Time}), an open-source Python package that implements a phenomenological model mapping classical spacetime curvature metrics (specifically the Frobenius norms of the Riemann and Ricci tensors) to single-qubit quantum decoherence channels. We emphasize that this mapping is a conceptual ansatz without a first-principles physical derivation. The project's main focus is mathematical consistency verification of the model's assumptions and regression testing rather than discovering new physics. 
...
\input{sections/00_theory_intent.tex}
