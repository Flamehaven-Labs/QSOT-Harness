...
def classify_background(bg) -> dict:
    ...
    is_einstein_space = (
        cosmological_constant is not None
        and einstein_residual_norm <= EINSTEIN_EPS
    )
    ...
    is_vacuum_like = (
        ...
        or (
            is_einstein_space
            and stress_energy_norm <= STRESS_EPS
        )
    )
...
def compute_beta_residuals(bg) -> dict:
    ...
    return {
        ...
        "brst_proxy_ok": cls["is_vacuum_like"] and cls["is_conformal_background"] and not cls["has_gravitational_anomaly"],
