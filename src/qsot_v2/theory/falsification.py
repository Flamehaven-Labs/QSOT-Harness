...
from qsot_v2.physics.channels import BACKGROUND_BUILDERS, MetricToChannelMap
...
def run_phase0_checks(sensitivity: float, boost_beta: float) -> Dict[str, Any]:
    ...
    for spec in scenarios:
        ...
        def single_step_map(rho: np.ndarray) -> np.ndarray:
            return sum(k @ rho @ k.conj().T for k in kraus_ops)
            
        def composed_map_fn(rho: np.ndarray) -> np.ndarray:
        ...
        lin_dev = float(np.linalg.norm(
            0.5 * single_step_map(test_states[0]) + 0.5 * single_step_map(test_states[1]) - 
        ...
        curr_rep2_ok = verifier.check_two_step_conditionability(rho_0, rho_2, single_step_map, single_step_map)
