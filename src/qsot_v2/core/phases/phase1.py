...
def run_phase1(ctx: PhaseContext) -> None:
    ...
    ctx.result.checks["flat_rest_purity_is_1"] = "PASS" if all(abs(p - 1.0) < 1e-12 for p in purities) else "FAIL"
    ctx.result.checks["flat_rest_entropy_is_0"] = "PASS" if all(e < 1e-12 for e in entropies) else "FAIL"
    ...
    ctx.result.checks["flat_ctc_is_clear"] = "PASS" if flat_verify["physics"]["ctc_status"] == "CLEAR" else "FAIL"
    ...
    ctx.result.checks["flat_boost_purity_is_1"] = "PASS" if all(abs(p - 1.0) < 1e-12 for p in purities_boost) else "FAIL"
